...
DockerTerminate()
{
    ...
    if [ "$SEND_ANONYMOUS_USAGE_INFO" = true ]; then
        ...
        if [[ $BUILD_URL == *"zalan.do"* ]]; then
            ...
            RANDOM_USER_GA_ID=docker-$(head -c 6 /dev/urandom  | base64)
        else
...
StartUp()
{
    ...
    if [ "$SEND_ANONYMOUS_USAGE_INFO" = true ]; then
        ...
        DisplayDataProcessingAgreement
        if [ "${ZALENIUM_DOCKER_PASSIVE}" == "true" ]; then
            DOCKER_CLIENT_VERSION=Docker
            ...
            CPU_NUMBER=0
            TOTAL_MEMORY=0.0
        elif [ ${KUBERNETES_ENABLED} == "false" ]; then
